...
function task(info: ToolProps<typeof TaskTool>) {
  const agent = info.input.subagent_type ? Locale.titlecase(info.input.subagent_type) : "Subagent"
...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    async function execute(sdk: OpencodeClient) {
      ...
      type MessageWithParts = { info: Message; parts: Array<{ type: string; text?: string }> }
      type PollMessage = Message | MessageWithParts

      const baselineMessages = await sdk.session
        .messages({ sessionID })
        .catch(() => ({ data: [] as PollMessage[] }))
      const baselineMessageCount = baselineMessages.data?.length ?? 0
      ...
      if (error && !hasAssistantText) {
        const deadline = Date.now() + 60_000
        while (Date.now() < deadline && !hasAssistantText) {
          const list = await sdk.session
            .messages({ sessionID })
            .catch(() => ({ data: [] as PollMessage[] }))
          const newMessages = (list.data ?? []).slice(baselineMessageCount)

          for (let i = newMessages.length - 1; i >= 0; i--) {
            const msg = newMessages[i]
            const info = "info" in msg ? msg.info : msg
