...
export namespace SessionPrompt {
  ...
  async function createUserMessage(input: PromptInput) {
    ...
    const requestedAgentName = input.agent ?? defaultAgentName
    const agent = (await Agent.get(requestedAgentName)) ?? (await Agent.get(defaultAgentName))

    if (!agent) {
      throw new Error(`Agent not found: ${requestedAgentName}`)
    }
