...
describe("runtime-fallback", () => {
  ...
  describe("session.error handling", () => {
    ...
    test("should skip MCP session errors even when they include retryable status text", async () => {
      const retriedModels: string[] = []
      const hook = createRuntimeFallbackHook(
        createMockPluginInput({
          session: {
            messages: async () => ({
              data: [{ info: { role: "user" }, parts: [{ type: "text", text: "search the docs" }] }],
            }),
      ...
      const sessionID = "test-session-mcp-error"
      ...
      await hook.event({
        event: {
          ...
          properties: {
            ...
            error: {
              message:
                'Search error (429): {"jsonrpc":"2.0","error":{"code":-32000,"message":"MCP server rate limit reached"}}',
            },
          },
        },
      })
      ...
      expect(skipLog).toBeDefined()
      expect(skipLog?.data).toMatchObject({ errorType: "tool_mcp_error" })
    })
    ...
    test("should skip fallback candidate when it matches current model", async () => {
      const hook = createRuntimeFallbackHook(createMockPluginInput(), {
        config: createMockConfig({ notify_on_fallback: false }),
      ...
      const sessionID = "test-session-skip-same-model"
      SessionCategoryRegistry.register(sessionID, "test")
    ...
    test("should not trigger fallback when provider-managed retry text is observed on the current model", async () => {
      const hook = createRuntimeFallbackHook(createMockPluginInput(), {
        config: createMockConfig({ notify_on_fallback: false }),
        pluginConfig: createMockPluginConfigWithCategoryFallback([
      ...
      const sessionID = "test-session-openai-auto-retry-skip-same"
      SessionCategoryRegistry.register(sessionID, "test")
        ...
        event: {
          ...
          properties: {
            info: {
              ...
              role: "assistant",
              model: "openai/gpt-5.3-codex",
              status: "The usage limit has been reached [retrying in 26s attempt #4]",
            },
          },
        },
      })
  ...
  describe("message.updated handling", () => {
    ...
    test("should advance fallback state from message.updated while awaiting fallback result", async () => {
      ...
      const processingLog = logCalls.find((c) =>
        c.msg.includes("Processing assistant error while awaiting fallback result")
      )
    ...
    test("should clear fallback timeout when persisted assistant tool progress is observed", async () => {
      const retriedModels: string[] = []
      ...
      const progressLog = logCalls.find((c) =>
        c.msg.includes("Assistant progress observed; cleared fallback timeout")
      )
    ...
    test("does not trigger fallback for MCP search rate limit errors", async () => {
      const retriedModels: string[] = []
  ...
  describe("model switching via chat.message", () => {
    ...
    test("keeps fallback model active while original model is in cooldown", async () => {
      const hook = createRuntimeFallbackHook(createMockPluginInput(), {
        config: createMockConfig({ notify_on_fallback: false, cooldown_seconds: 60 }),
        pluginConfig: createMockPluginConfigWithCategoryFallback(["github-copilot/claude-sonnet-4.5", "openai/gpt-5.2"]),
      })
      const sessionID = "test-session-sticky-fallback"
      SessionCategoryRegistry.register(sessionID, "test")
  ...
  describe("race condition guards", () => {
    test("session.error while awaiting fallback result advances to next model", async () => {
      let promptCallCount = 0
      const never = new Promise<never>(() => {})
      ...
      //#then
      const inFlightProcessingLog = logCalls.find((call) =>
        call.msg.includes("Processing session.error while awaiting fallback result")
      )
      expect(inFlightProcessingLog).toBeDefined()
    ...
    test("assistant error without retry signal advances while fallback request is in flight", async () => {
      let promptCallCount = 0
      const never = new Promise<never>(() => {})
      ...
      const sessionID = "test-race-message-updated-in-flight"
      SessionCategoryRegistry.register(sessionID, "test")
        ...
        event: {
          ...
          properties: {
            info: {
              ...
              error: { statusCode: 429, message: "This request would exceed your account's rate limit. Please try again later." },
            },
          },
        },
      })
      ...
      const processingLog = logCalls.find((call) =>
        call.msg.includes("Processing assistant error while awaiting fallback result")
      )
      expect(processingLog).toBeDefined()
