...
describe("createAnthropicContextWindowLimitRecoveryHook", () => {
  ...
  test("does not skip compaction on session.idle when summary exists but session is still over token limit", async () => {
    //#given
    ...
    parseAnthropicTokenLimitErrorMock.mockReturnValueOnce({
      providerID: "anthropic",
      modelID: "claude-sonnet-4-6",
      currentTokens: 242141,
      maxTokens: 200000,
    })
    ...
    try {
      ...
      await hook.event({
        event: {
          ...
          properties: {
            ...
            error: "prompt is too long: 242141 tokens > 200000 maximum",
          },
        },
      })
      ...
      await hook.event({
        event: {
          type: "session.idle",
          properties: { sessionID: "session-over-limit" },
        },
      })

      //#then
      expect(executeCompactMock).toHaveBeenCalledTimes(1)
      expect(executeCompactMock.mock.calls[0]?.[0]).toBe("session-over-limit")
    } finally {
      restore()
    }
  })
