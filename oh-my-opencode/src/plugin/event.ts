...
export function createEventHandler(args: {
  ...
  const hasConfiguredFallbackModels =
    Object.values(args.pluginConfig.agents ?? {}).some(
      (agentConfig) => Array.isArray(agentConfig?.fallback_models) && agentConfig.fallback_models.length > 0,
    ) ||
    Object.values(args.pluginConfig.categories ?? {}).some(
      (categoryConfig) =>
  ...
  const isRuntimeFallbackEnabled =
    ...
    (typeof args.pluginConfig.runtime_fallback === "boolean"
      ...
      : (args.pluginConfig.runtime_fallback?.enabled ?? hasConfiguredFallbackModels));
