...
export function createSessionHooks(args: {
  ...
  const effectiveRuntimeFallbackConfig = runtimeFallbackConfig ?? {
    enabled: hasConfiguredFallbackModels,
  }

  if (
    runtimeFallbackConfig &&
    typeof runtimeFallbackConfig === "object" &&
    runtimeFallbackConfig.enabled === undefined &&
    hasConfiguredFallbackModels
  ) {
    effectiveRuntimeFallbackConfig.enabled = true
  }

  const runtimeFallback = isHookEnabled("runtime-fallback")
    ? safeHook("runtime-fallback", () =>
        createRuntimeFallbackHook(ctx, {
          config: effectiveRuntimeFallbackConfig,
          pluginConfig,
        }))
    : null
  return {
